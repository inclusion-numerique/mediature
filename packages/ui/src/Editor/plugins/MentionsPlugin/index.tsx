...
const dummyLookupService = {
  search(string: string, callback: (results: Array<string>) => void): void {
    setTimeout(() => {
      const items: string[] = []; // This should be gotten from the server in case we want to give suggestions
      const results = items.filter((mention) => mention.toLowerCase().includes(string.toLowerCase()));
      callback(results);
    }, 500);
  },
};
