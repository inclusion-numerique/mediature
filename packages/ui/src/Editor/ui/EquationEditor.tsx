...
export default function EquationEditor({ equation, setEquation, inline, inputRef }: BaseEquationEditorProps): JSX.Element {
  ...
  return inline ? (
    <InlineEquationEditor {...props} inputRef={inputRef as RefObject<HTMLInputElement>} />
  ) : (
    <BlockEquationEditor {...props} inputRef={inputRef as RefObject<HTMLTextAreaElement>} />
  );
}
...
function InlineEquationEditor({ equation, onChange, inputRef }: EquationEditorImplProps): JSX.Element {
  return (
    <span className="EquationEditor_inputBackground">
      ...
      <input className="EquationEditor_inlineEditor" value={equation} onChange={onChange} autoFocus={true} ref={inputRef} />
      <span className="EquationEditor_dollarSign">$</span>
    </span>
  );
}
